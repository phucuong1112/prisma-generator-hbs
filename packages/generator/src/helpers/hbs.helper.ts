...
export function registerHandlebarsHelpers(): void {
  ...
  Handlebars.registerHelper('includes', function(list: string | any[], item: any) {
  ...
  Handlebars.registerHelper('find', function(array: any[], propertyPath: string | ((item: any) => boolean), value?: any) {
  ...
  Handlebars.registerHelper('generateZodSchemaProperties', function(modelInfo, modelClassName) {
    if (!modelInfo || !modelInfo.fields) return '';
    
    const excludedFields = ['id', 'createdAt', 'updatedAt', 'deletedAt', 'createdBy', 'updatedBy'];
    ...
    return fieldsToInclude.map((field: any, index: number) => {
      ...
      switch (field.type) {
        case 'String':
          ...
          if (field.nativeType && field.nativeType[0] === 'VarChar' && field.nativeType[1] && field.nativeType[1][0]) {
  ...
  Handlebars.registerHelper('generateResponseSchemaProperties', function(modelInfo, modelClassName) {
    ...
    return standardFields.map((field, index) => {
      let result = `    ${field.name}: `;
  ...
  Handlebars.registerHelper('generateSearchSchemaProperties', function(modelInfo, modelClassName, modelVarName) {
    if (!modelInfo || !modelInfo.fields) return '';
    ...
    const searchableFields = modelInfo.fields.filter((field: any) => {
      ...
      const metadataFields = ['createdAt', 'updatedAt', 'deletedAt', 'createdBy', 'updatedBy'];
      ...
      if (field.type === 'String') {
        ...
        const meaningfulFields = ['name', 'title', 'slug', 'email', 'username', 'code', 'reference', 'description'];
        if (field.isUnique || meaningfulFields.some(name => field.name.toLowerCase().includes(name.toLowerCase()))) {
          return true;
        }
      }
      
      // Include other fields that might be useful for filtering
      if (['status', 'type', 'category'].some(name => field.name.toLowerCase().includes(name.toLowerCase()))) {
        return true;
      }
      ...
      if (field.type === 'DateTime' && !['createdAt', 'updatedAt', 'deletedAt'].includes(field.name)) {
        return true;
      }
    ...
    return searchableFields.map((field: any, index: number) => {
      let result = `  ${field.name}: `;
  ...
  if (fs.existsSync(helpersDir)) {
    ...
    helperFiles.forEach(file => {
      if (file.endsWith('.hbs')) {
        ...
        const partialContent = fs.readFileSync(path.join(helpersDir, file), 'utf8');
